...
import type { MessageToolTransport, CreateMessageBody, MessageUploadFile } from "./message-tool-transport";
...
function sanitizeFilename(raw: string): string {
  return raw.replace(/\.\./g, "_").replace(/[/\\]/g, "_").replace(/[\x00-\x1f]/g, "").trim() || "attachment";
}

function decodeBase64File(raw: string, filename: string): MessageUploadFile {
  let b: Buffer;
  try { b = Buffer.from(raw, "base64"); } catch { throw new Error(`attachments[].content_base64 for ${filename} is not valid base64`); }
  if (b.length === 0) throw new Error(`attachments[].content_base64 for ${filename} decoded to empty buffer`);
...
export async function executeMessageToolAction(
  ...
  try {
    if (a === "get") {
      ...
      if (!targetChannel) {
        return { ok: false, error: "get needs a channel: set channel_id or use a session bound to a channel", sessionId: sid };
      }
      ...
      if (singleId) {
        ...
        return { ok: true, channel_id: targetChannel, messages: [summarizeApiMessage(raw)] };
      }
      if (anchor) {
        if (dirRaw !== "before" && dirRaw !== "after" && dirRaw !== "around") {
          return { ok: false, error: "anchor_message_id requires list_direction: before | after | around" };
        }
        ...
        return { ok: true, channel_id: targetChannel, messages: rows.map(summarizeApiMessage) };
      }
      if (dirRaw) {
        return { ok: false, error: "list_direction without anchor_message_id; omit both for latest messages, or set anchor_message_id" };
      }
    ...
    if (a === "post") {
      ...
      if (replyTo && !x.replies) return { ok: false, error: "reply_to_message_id not supported on this platform" };

      const attRaw = args.attachments;
      let files: MessageUploadFile[] = [];
      if (attRaw !== undefined) {
        if (!x.attachments) return { ok: false, error: "attachments not supported on this platform" };
        if (!Array.isArray(attRaw)) return { ok: false, error: "attachments must be an array" };
        for (const row of attRaw) {
          if (row === null || typeof row !== "object" || Array.isArray(row)) return { ok: false, error: "each attachment must be an object" };
          const o = row as Record<string, unknown>;
          const filename = str(o.filename, "attachments[].filename");
          const b64 = o.content_base64;
          if (typeof b64 !== "string" || !b64.trim()) return { ok: false, error: "attachments[].content_base64 required" };
          files.push(decodeBase64File(b64.trim(), filename));
        }
      }
      if (!content && files.length === 0) return { ok: false, error: "post requires content and/or attachments" };
    ...
    if (a === "create_thread") {
      if (!x.threadCreate) return { ok: false, error: "create_thread not supported on this platform" };
      const messageId = str(args.message_id, "message_id");
      ...
      if (dur !== undefined) {
        if (dur !== 60 && dur !== 1440 && dur !== 4320 && dur !== 10080) {
          return { ok: false, error: "auto_archive_duration_minutes must be one of 60, 1440, 4320, 10080" };
        }
        auto_archive_duration = dur;
      }
      const { id } = await t.createThreadFromMessage(channelId, messageId, {
        name, ...(auto_archive_duration !== undefined ? { auto_archive_duration } : {}),
      });
      return { ok: true, thread_id: id, parent_channel_id: channelId, message_id: messageId };
    }

    if (a === "delete_thread") {
      if (!x.threadDelete) return { ok: false, error: "delete_thread not supported on this platform" };
    ...
    if (a === "choice") {
      ...
      for (const c of choicesRaw) {
        ...
        if (!emoji || !label) return { ok: false, error: "each choice requires non-empty emoji and label" };
      ...
      for (const c of choices) {
        try { await t.createMessageReaction(channelId, id, c.emoji); } catch { /* best-effort */ }
      }
    ...
    if (a === "reactions") {
      ...
      if (emojiFilter) {
        ...
        return { ok: true, message_id: messageId, channel_id: channelId, reactions: [{ emoji: emojiFilter, count: summarized.length, users: summarized }] };
      }
    ...
    if (a === "search") {
      ...
      if (!guildId) return { ok: false, error: "search requires a guild context; not available for DM sessions" };
      ...
      if (fromMe === true && !authorIds) {
        return { ok: false, error: "from_me requires the bot's own user id; pass it via author_id instead" };
      }

      let channelIds: string[] | undefined;
      if (Array.isArray(channelIdsRaw) && channelIdsRaw.length > 0) {
        channelIds = channelIdsRaw.filter((c: unknown) => typeof c === "string" && c.trim()).map((c: unknown) => (c as string).trim());
      } else {
      ...
      return { ok: true, channel_id: channelId, total_results: searchResult.total_results, messages };
    }

    if (a === "attachment-download") {
      if (!x.attachmentDownload) return { ok: false, error: "attachment-download not supported on this platform" };
      if (!deps.downloadFile) return { ok: false, error: "attachment download not configured (no downloadFile handler)" };
      ...
      if (rawAttachments.length === 0) return { ok: false, error: "message has no attachments", message_id: messageId };

      let attachment: Record<string, unknown> | undefined;
      if (filenameFilter) {
        attachment = rawAttachments.find((a: Record<string, unknown>) => typeof a.filename === "string" && a.filename === filenameFilter);
        if (!attachment) {
          const available = rawAttachments.map((a: Record<string, unknown>) => a.filename).filter((f: unknown) => typeof f === "string");
          return { ok: false, error: `attachment "${filenameFilter}" not found`, available_filenames: available, total_attachments: rawAttachments.length };
        }
      } else {
        const idx = typeof indexRaw === "number" && Number.isFinite(indexRaw) ? Math.trunc(indexRaw) : 0;
        if (idx < 0 || idx >= rawAttachments.length) {
          return { ok: false, error: `attachment index ${idx} out of range (0..${rawAttachments.length - 1})`, total_attachments: rawAttachments.length };
        }
      ...
      if (sizeBytes !== undefined && sizeBytes > maxSize) {
        return { ok: false, error: `attachment too large (${sizeBytes} bytes, max ${maxSize})`, filename, size: sizeBytes };
      }
      ...
      return { ok: true, path: finalPath, filename: safeName, mimeType: contentType, size: bytesWritten, totalAttachments: rawAttachments.length } satisfies AttachmentDownloadResult;
    }
