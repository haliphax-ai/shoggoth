...
function createDiscordDeliveryResolver(configRef: { current: any }): PlatformDeliveryResolver {
  return {
    ...
    resolveSessionForInbound(identifiers, config) {
      return resolveSessionForChannel(
    ...
    hooks: {
      ...
      async "platform.start"(ctx) {
        const {
          ...
          setMessageToolContext,
          setPlatformAdapter,
        } = ctx as PlatformStartCtx;
        ...
        state.reactionPassthroughRef.current = (ev) => {
          ...
          if (!sessionId) {
            logger.debug("reaction.passthrough.no_session", {
              channelId: ev.channelId,
            });
            return;
          }

          const emojiStr = ev.emoji.id
            ? `<:${ev.emoji.name ?? "_"}:${ev.emoji.id}>`
            : (ev.emoji.name ?? "");
          if (!emojiStr) return;
          ...
          void (async () => {
            try {
              ...
              logger.warn("reaction.passthrough.fetch_failed", {
                err: String(e),
                messageId: ev.messageId,
              });
            }
